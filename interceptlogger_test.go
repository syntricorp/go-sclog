...
func TestInterceptLogger(t *testing.T) {
	...
	t.Run("Sinks accept different log formats", func(t *testing.T) {
		...
		expected := fmt.Sprintf("[INFO]  go-sclog/interceptlogger_test.go:%d: this is a test: who=caller\n", line-1)
	...
	t.Run("includes the caller location", func(t *testing.T) {
		...
		expected := fmt.Sprintf("[INFO]  go-sclog/interceptlogger_test.go:%d: test: this is test: who=programmer why=\"testing is fun\"\n", line-1)
		assert.Equal(t, expected, rest)
