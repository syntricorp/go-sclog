...
var Analyzer = &analysis.Analyzer{
	Name:     "sclogvet",
	Doc:      "check sclog invocations",
	Requires: []*analysis.Analyzer{inspect.Analyzer},
	Run:      run,
}

var checkSCLogFunc = map[string]bool{
	"Trace": true,
	"Debug": true,
	"Info":  true,
	"Warn":  true,
...
func run(pass *analysis.Pass) (interface{}, error) {
	for _, f := range pass.Files {
		ast.Inspect(f, func(n ast.Node) bool {
			...
			if !isNamedType(recv.Type, "github.com/syntricorp/go-sclog", "Logger") &&
				!isNamedType(recv.Type, "github.com/syntricorp/go-sclog", "InterceptLogger") {
				return true
			}

			if _, ok := checkSCLogFunc[fun.Sel.Name]; !ok {
				return true
			}
