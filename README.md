...
[godocs]: https://godoc.org/github.com/syntricorp/go-sclog

`go-sclog` is a package for Go that provides a simple key/value logging
interface for use in development and production environments.

It provides logging levels that provide decreased output based upon the
desired amount of output, unlike the standard library `log` package.

It provides `Printf` style logging of values via `sclog.Fmt()`.
...
Install using `go get github.com/syntricorp/go-sclog`.

Full documentation is available at
http://godoc.org/github.com/syntricorp/go-sclog
...
```go
sclog.Default().Info("hello world")
```
...
```go
appLogger := sclog.New(&sclog.LoggerOptions{
	Name:  "my-app",
	Level: sclog.LevelFromString("DEBUG"),
})
```
...
```go
totalBandwidth := 200
appLogger.Info("total bandwidth exceeded", "bandwidth", sclog.Fmt("%d GB/s", totalBandwidth))
```
...
`sclog.Logger` by calling the `StandardLogger()` method. This allows you to use
it with the familiar `Println()`, `Printf()`, etc. For example:

```go
stdLogger := appLogger.StandardLogger(&sclog.StandardLoggerOptions{
	InferLevels: true,
})
// Printf() is provided by stdlib log.Logger interface, not sclog.Logger
...
log.SetOutput(appLogger.StandardWriter(&sclog.StandardLoggerOptions{InferLevels: true}))
log.SetPrefix("")
log.SetFlags(0)
